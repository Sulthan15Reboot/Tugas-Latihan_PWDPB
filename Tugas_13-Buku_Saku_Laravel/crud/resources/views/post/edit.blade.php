                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('post.update', $post) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-2">
                            ...
                            <textarea name="body" id="body" rows="4" class="form-input rounded-md shadow-sm w-full @error('body') border border-red-500 @enderror" placeholder="enter blog body">{{ old('body') ?? $post->body }}</textarea>
                            @error('body')
                            <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
